        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.stylists.update", [$stylist->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">Nama</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $stylist->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone" id="phone" value="{{ old('phone', $stylist->phone) }}">
                            @if($errors->has('phone'))
                        ...
                        <div class="form-group {{ $errors->has('skills') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="skills" id="skills">
                                ...
                                @foreach(App\Models\Stylist::SKILLS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('skills', $stylist->skills) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('barbershop') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="barbershop_id" id="barbershop_id">
                                @foreach($barbershops as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('barbershop_id') ? old('barbershop_id') : $stylist->barbershop->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
