                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.barbershops.update", [$barbershop->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $barbershop->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="address" id="address" required>{{ old('address', $barbershop->address) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone" id="phone" value="{{ old('phone', $barbershop->phone) }}">
                            @if($errors->has('phone'))
                        ...
                        <div class="form-group {{ $errors->has('services') ? 'has-error' : '' }}">
                            <label class="required" for="services">Jasa-Jasa</label>
                            ...
                            <select class="form-control select2" name="services[]" id="services" multiple required>
                                @foreach($services as $id => $service)
                                    <option value="{{ $id }}" {{ (in_array($id, old('services', [])) || $barbershop->services->contains($id)) ? 'selected' : '' }}>{{ $service }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('about_barber') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="about_barber" id="about_barber" required>{{ old('about_barber', $barbershop->about_barber) }}</textarea>
